...
function initItemList()
{
        ...
        for (const ele in list)
        {
            if (typeof (list[ele]) === "object")
            {
                if (list[ele].parentElement.children.length === 9) {
                    ...
                    obj1[list[ele].innerText] = nextchild2.innerText;
                    obj2[list[ele].innerText] = nextchild3.innerText;
                }
                else
                {
                    const nextchild1 = list[ele].nextElementSibling;
                    ...
                    obj1[list[ele].innerText] = nextchild1.innerText;
                    obj2[list[ele].innerText] = nextchild2.innerText;
                }
            }
        }
    ...
    const typeArr2 = ["杖-回数(販売)", "杖-回数(買取)", "壺-回数(販売)", "壺-回数(買取)"];
...
input.addEventListener('input', inputEventInputElement);
input.addEventListener('focus', focusEventInputElement);
